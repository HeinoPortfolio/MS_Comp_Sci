...
# Discussion and Conclusion 
 ...
 This project proposed was to investigate if there was a relationship between changes in unemployment rate and the crime rate. This project resulted in numeric and graphical data that can be used to answer the following questions:
...
While it might be self evident that with an increase of unemployment there would be an increase in the crime rate. We can use the analysis that resulted in the graphs and the numerical data to see if there is a correlation.  There were many methods employed to see if there was a correlation between the two. 

Looking at the statistics that can be found in Section 3.1 EDA Analysis \@ref(eda-analysis),  we can see in tables that show the some statistical measurements for the data.  The tables and graphs show the quantiles, the mean, maximum, minimum, and any outliers for the available data. Table \@ref(tab:eda-analysis-unemp-two) shows the grouped data for all the years within the dataset. In Figure 3.1  \@ref(fig:reg-unemp-box).  Based on the boxplots regions 3 and 4 seems to have extreme range in the unemployment rate for the time period. 
If we look at the data for each year as given in Table \@ref(tab:eda-analysis-unemp-two), the regions 3 and 4 usually have the highest mean unemployment rate over the time period. To better illustrate the measurements that were given in the table, it may be helpful to look at a boxplot of the data over time.  Using a faceted boxplot we can visualize the data for the regions for each year that there is data. Looking at Figure \@ref(fig:reg-yr-unemp-box) we can see that regions 2 and 4 do show the extremes in the unemployment rate. 
