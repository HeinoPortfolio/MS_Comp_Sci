...
Examining the statistics that can be found in Section \@ref(eda-analysis) EDA Analysis, we can see tables that show some statistical measurements for the data.  The tables and graphs show the quantiles, the mean, maximum, minimum, and any outliers for the available data. Table \@ref(tab:eda-analysis-unemp-one) Regional Unemployment Statistics. Shows the grouped data for all the years within the data set. In Figure \@ref(fig:reg-unemp-box) Unemployment Rate by Region. Based on the box plots regions 3 and 4 seem to have an extreme range in the unemployment rate for the period. 
